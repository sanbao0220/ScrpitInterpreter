...
├── .idea
├── scripts
│   ├── {} jd_service_script.json (M)
│   ├── jd_service_script.txt (M)
│   ├── {} normal_script.json (M)
│   ├── {} text_script.json (M)
│   └── text_script.txt (M)
├── src
│   ├── conponents  # 注意：可能是拼写错误（应为 components）
│   │   ├── __pycache__
│   │   ├── __init__.py (U)
│   │   ├── Ast1.py (U)
│   │   ├── data_api.py (M)
│   │   ├── get_jsonScript.py (M)
│   │   └── ScriptInterpreter.py (U)
│   │   └── __init__.py (U)  # 若为子包的 init，需结合实际结构
│   └── （注：src 下只有 conponents 目录，无其他同级项）
├── user.data / jd_service_script
│   └── {} user_zhangsan.json (U)
├── 智能客服机器人架构设计.md (M)
├── CHANGELOG.md (U)
├── csbot.db
└── img.png
